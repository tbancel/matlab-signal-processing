function varargout =sigma_eegfc(varargin)
% sigma_eegfc - the main Graphical User Interface (GUI) environment 
%this is a modified version of eegfc for the use in the sigma Toolbox
%modified by Joffrey THOMAS : joffrey.thomas.18@eigsi.fr

%the authors of the of the original function:
...
%     1. type
%         >> sigma_eegfc
%        or call sigma_eegfc to start the popup GUI
%       
%     2. type 
%         >> sigma_eegfc(EEG)
%         or call sigma_eegfc(EEG) to start the popup GUI with EEG structure. 
%         The EEG structure should be pre-exported by the sigma_eegfc GUI 
%         or made by 
%         >> EEG = pop_txtreader  
%         or
%         >> EEG = pop_matreader
...
%           
%      3. call sigma_eegfc from the main econnectome GUI ('Menu bar -> EEG')
%
% Description:
%  sigma_eegfc is the main GUI for EEG functional connectivity. Multi-channel EEG 
...
%
% Reference for sigma_eegfc() (please cite):
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @sigma_eegfc_OpeningFcn, ...
                   'gui_OutputFcn',  @sigma_eegfc_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before sigma_eegfc is made visible.
function sigma_eegfc_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to sigma_eegfc (see VARARGIN)





% Choose default command line output for sigma_eegfc
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes sigma_eegfc wait for user response (see UIRESUME)
...
%global init_parameter_sigma_eegfc;
% Mettre un test
init_parameter_sigma_eegfc = varargin{1};
%evalin('base', 'init_parameter');

handles.init_parameter_sigma_eegfc=init_parameter_sigma_eegfc;
guidata(hObject,handles)

%% ************************* modification apporté ici
% selection of the epoch and the subject to visualize from the initialized
% parameters
%epoch =init_parameter_eegfc.epoch_to_display;   
subject = init_parameter_sigma_eegfc.subject_to_display;

if sum(init_parameter_sigma_eegfc.subject==subject)==1
    %DEBUT******************* version initiale
    %We convert the signal
        [eeg2,init] = Sigma_converting_data(init_parameter_sigma_eegfc);
        eeg = struct('EEG',eeg2);
        EEG = Sigma_converting_data2(eeg);
    %FIN ******************* version initiale   
else
    msgbox('This selected subject does not exist in your data base (init_parameter)!','','help','modal');
    return;
end
%% FIN ************************* modification apporté ici
...
init_parameter_sigma_eegfc.available_epochs=ep;
...
nEEGColor = [0.0,1.0,0.0];


t = 0:round(1/200):1;
randData = sin(2*pi*300*t);

ax8 = findobj(gcf,'tag','axes8');
ax7 = findobj(gcf,'tag','axes7');
ax6 = findobj(gcf,'tag','axes6');

 set(ax8, 'userdata',randData ,'color',goodColor);
 set(ax7, 'userdata',randData ,'color', BadColor);
 set(ax6, 'userdata',randData ,'color',nEEGColor);
% Update handles structure

assignin('base','init_parameter_sigma_eegfc',init_parameter_sigma_eegfc);

% --- Outputs from this function are returned to the command line.
function varargout = sigma_eegfc_OutputFcn(hObject, eventdata, handles) 
...
if isempty(linehandle)
    ...
    plot(xpos, ypos, 'color', tmpcolor, 'clipping','on', 'tag', 'linetag'); drawnow;
else
...
if isempty(pointhandle)
    % plot(x,y,'mo','MarkerFaceColor',[0.49,1.0,0.63],'MarkerSize',8,'EraseMode', 'xor','tag', 'pointhandle'); drawnow;
    plot(x,y,'mo','MarkerFaceColor',[0.49,1.0,0.63],'MarkerSize',8,'tag', 'pointhandle'); drawnow;
else
...
default = [];
[sel,ok] = listdlg('ListString',EEG.labels,'Name','Interested Channels','InitialValue',default);
if ok == 0 || isempty(sel)
    return;
end
EEG.data=EEG.data(sel,:);
if isempty(EEG) | isempty(EEG.data)
    helpdlg('Please import EEG data!');
    return;
end  
...
if strcmp(selectype,'normal')
    ...
    if isempty(linehandle)
       % plot(xpos, ypos, 'color', tmpcolor, 'clipping','on','EraseMode', 'xor', 'tag', 'currentline');
        plot(xpos, ypos, 'color', tmpcolor, 'clipping','on', 'tag', 'currentline'); drawnow;
    else
       set(linehandle,'xdata',xpos,'ydata',ypos);
       drawnow;
    end
...
% y labels  % Fait attention il ne récupére pas toutes les electrodes dans
% EEG
...
if options.epochstart>xlow && options.epochstart<xhigh
    ...
    if isempty(linehandle)
        % plot(xpos, ypos, 'color', tmpcolor, 'clipping','on','EraseMode', 'xor', 'tag', 'leftline');
         plot(xpos, ypos, 'color', tmpcolor, 'clipping','on','tag', 'leftline'); drawnow;
    else
       set(linehandle,'xdata',xpos,'ydata',ypos);
       drawnow;
    end
...
if options.epochend>xlow && options.epochend<xhigh
    ...
    if isempty(linehandle)
       % plot(xpos, ypos, 'color', tmpcolor, 'clipping','on','EraseMode', 'xor', 'tag', 'rightline');
        plot(xpos, ypos, 'color', tmpcolor, 'clipping','on','tag', 'rightline'); drawnow;
    else
       set(linehandle,'xdata',xpos,'ydata',ypos);
       drawnow;
    end
...
init_parameter_sigma_eegfc = handles.init_parameter_sigma_eegfc;
loadeeg = pop_matreader(init_parameter_sigma_eegfc);
...
for i = points
    ...
    if isempty(linehandle)
        ...
        plot(xpos, ypos, 'color', linecolor, 'clipping','on', 'tag', 'currentline'); drawnow;
    else
       set(linehandle,'xdata',xpos,'ydata',ypos);
    end
    drawnow; 
...
if isempty(linehandle)
   %plot(xpos, ypos, 'color', tmpcolor, 'clipping','on','EraseMode', 'xor', 'tag', 'leftline','LineStyle','--');
    plot(xpos, ypos, 'color', tmpcolor, 'clipping','on', 'tag', 'leftline','LineStyle','--'); drawnow;
else
   set(linehandle,'xdata',xpos,'ydata',ypos);
   drawnow;
end
...
if isempty(linehandle)
  % plot(xpos, ypos, 'color', tmpcolor, 'clipping','on','EraseMode', 'xor', 'tag', 'rightline','LineStyle','--');
   plot(xpos, ypos, 'color', tmpcolor, 'clipping','on', 'tag', 'rightline','LineStyle','--'); drawnow;
else
   set(linehandle,'xdata',xpos,'ydata',ypos);
   drawnow;
end
...
%init_parameter_sigma_eegfc = handles.init_parameter_sigma_eegfc;
init_parameter_sigma_eegfc = evalin('base', 'init_parameter_sigma_eegfc');
...
init_parameter_sigma_eegfc.epoch_to_display=epoch_to_display;
%handles.init_parameter_sigma_eegfc = init_parameter_sigma_eegfc;
assignin('base','init_parameter_sigma_eegfc',init_parameter_sigma_eegfc);
...
init_parameter_sigma_eegfc.subject_to_display = subject;
...
bad=init.bad;
init_parameter_sigma_eegfc.bad=bad;
...
if(epoch_to_display>av_epochs)
    ...
    init_parameter_sigma_eegfc.isok='false';
    pause(4);
...
%this callback is for he subject selection.
...
init_parameter_sigma_eegfc.subject_to_display=Subject_to_display;
%handles.init_parameter_sigma_eegfc = init_parameter_sigma_eegfc;
assignin('base','init_parameter_sigma_eegfc',init_parameter_sigma_eegfc);
...
Sigma_compute_topoplot(init_parameter_sigma_eegfc,currentpoint);
...
Sigma_compute_multiplot(init_parameter_sigma_eegfc,start_sample,end_sample)
...
end_sample = endpoint;
 ...
 Sigma_compute_ERP_visualization( init_parameter_sigma_eegfc,start_sample,end_sample )
