function saen = fc_sample_entropy( dim, r, dataM, tau )
% function saen = SampEn( dim, r, dataM, tau )
...
if tau >= 1, temp = downsample(dataM(1,:), tau); end
N = length(temp);
numChan = size(dataM,1);
correl = zeros(1,2);

dataMat = zeros(numChan*(dim+1),N-dim);
for chan = 1:numChan;
data = dataM(chan,:);
if tau > 1, data = downsample(data, tau); end
monoDataMat = zeros(dim+1,N-dim);
for i = 1:dim+1
monoDataMat(i,:) = data(i:N-dim+i-1);
end
dataMat(((dim+1)*(chan-1)+1):chan*(dim+1),:) = monoDataMat;
end
