...
%scores = nan(maxFeatNum,sum(nb_epochs));
...
for subject = 1:nb_subject
    ...
    epoch_to_romove=epoch:epoch+nb_epochs(subject)-1;  
    %epoch_to_romove
    ...
    for feat = 1:maxFeatNum
        ...
        %scores(feat,epoch_to_romove) = sc(2);
        scores_nc(feat,epoch_to_romove) = sc(1); % negative classe's score
        scores_pc(feat,epoch_to_romove) = sc(2); % negative classe's score
