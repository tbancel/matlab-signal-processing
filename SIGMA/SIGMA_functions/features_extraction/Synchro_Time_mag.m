function features_results=Synchro_amplitude_mag(init_parameter,~,features_results,s_EEG,Nsubj,Nepochs,Nmethode)
...
o_synchro_time_mag_ch=[];
temp0=[];
if (Nepochs==1)
o_synchro_time_mag_epo=[];
if Nsubj==1 
features_results.o_synchro_time_mag=[];
end
end
...
o_synchro_time_mag0=sqrt(sum(fdata_1.^2)./sum(fdata_2.^2));
o_synchro_time_mag_ch=[o_synchro_time_mag_ch; o_synchro_time_mag0];
...
o_synchro_time_mag0=sqrt(sum(i_EEG_1.^2)./sum(i_EEG_2.^2));
o_synchro_time_mag_ch=[o_synchro_time_mag_ch; o_synchro_time_mag0];
% Track the identification of the band
if ((Nsubj==1)&&(Nepochs==1))
...
o_synchro_time_mag_epo=o_synchro_time_mag_ch;  

features_results.o_synchro_time_mag=[features_results.o_synchro_time_mag o_synchro_time_mag_epo];

%features_results.o_synchro_time_mag=o_synchro_time_mag;

features_results.channels_combinaison=channels_combinaison;
if ((Nsubj==1)&&(Nepochs==1))
features_results.o_synchro_time_mag_band=temp0;
end    
