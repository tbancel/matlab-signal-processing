function features_results=Synchro_amplitude_emax(init_parameter,~,features_results,s_EEG,Nsubj,Nepochs,Nmethode)

%==========================================================================
%% This script is part of the BCI toolbox, it's used by the main function to compute the "mean value for each channels"
%==========================================================================
% This script compute the EMAX between two samples 
o_synchro_time_emax_ch=[];
temp0=[];
if (Nepochs==1)
o_synchro_time_emax_epo=[];
if Nsubj==1 
features_results.o_synchro_time_emax=[];
end
end
...
o_synchro_time_emax0=(max(fdata_2-fdata_1))/((max(fdata_2))-(min(fdata_1)));
...
o_synchro_time_emax0=(max(i_EEG_2-i_EEG_1))/((max(i_EEG_2))-(min(i_EEG_1)));
o_synchro_time_emax_ch=[o_synchro_time_emax_ch; o_synchro_time_emax0];
% Track the identification of the band
if ((Nsubj==1)&&(Nepochs==1))
...
features_results.o_synchro_time_emax=[features_results.o_synchro_time_emax o_synchro_time_emax_epo];
