function features_results=Synchro_amplitude_mean_coh(init_parameter,~,features_results,s_EEG,Nsubj,Nepochs,Nmethode)
...
o_synchro_amp_mean_coh_ch=[];
temp0=[];
if (Nepochs==1)
o_synchro_amp_mean_coh_epo=[];
    if Nsubj==1 
    features_results.o_synchro_amp_mean_coh=[];
    end
end
                     ...
                     if downsampling==1; % 0-> no downsampling // 1-> downsampling
                        ...
                        if (apply_filter==1)
                            for Nband=1:init_parameter.nb_bands
                                ...
                                mean_coh = mean(mscohere(fdata_1, fdata_2,[],[],init_parameter.sampling_rate));
                               % cpt
                              o_synchro_amp_mean_coh0=mean_coh;
                              o_synchro_amp_mean_coh_ch=[o_synchro_amp_mean_coh_ch; o_synchro_amp_mean_coh0];
                                ...
                                mean_coh = mean(mscohere(i_EEG_1, i_EEG_2,[],[],init_parameter.sampling_rate));
...
o_synchro_amp_mean_coh_epo = o_synchro_amp_mean_coh_ch;  

features_results.o_synchro_amp_mean_coh=[features_results.o_synchro_amp_mean_coh o_synchro_amp_mean_coh_epo];

%features_results.o_synchro_amp_mean_coh=o_synchro_amp_mean_coh;

features_results.channels_combinaison=channels_combinaison;
if ((Nsubj==1)&&(Nepochs==1))
    features_results.o_synchro_amp_mean_coh_band=temp0;
end    
