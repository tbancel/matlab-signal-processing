function features_results=Synchro_amplitude_rdm(init_parameter,~,features_results,s_EEG,Nsubj,Nepochs,Nmethode)
...
o_synchro_time_rdm_ch=[];
temp0=[];
if (Nepochs==1)
o_synchro_time_rdm_epo=[];
if Nsubj==1 
features_results.o_synchro_time_rdm=[];
end
end
...
o_synchro_time_rdm0=sqrt(sum((((fdata_1)./(sqrt(sum(fdata_1.^2))))-((fdata_2)./(sqrt(sum(fdata_2.^2))))).^2));
o_synchro_time_rdm_ch=[o_synchro_time_rdm_ch; o_synchro_time_rdm0];
...
o_synchro_time_rdm0=sqrt(sum((((i_EEG_1)./(sqrt(sum(i_EEG_1.^2))))-((i_EEG_2)./(sqrt(sum(i_EEG_2.^2))))).^2));
o_synchro_time_rdm_ch=[o_synchro_time_rdm_ch; o_synchro_time_rdm0];
% Track the identification of the band
if ((Nsubj==1)&&(Nepochs==1))
...
o_synchro_time_rdm_epo=o_synchro_time_rdm_ch;  

features_results.o_synchro_time_rdm=[features_results.o_synchro_time_rdm o_synchro_time_rdm_epo];

%features_results.o_synchro_time_rdm=o_synchro_time_rdm;

features_results.channels_combinaison=channels_combinaison;
if ((Nsubj==1)&&(Nepochs==1))
features_results.o_synchro_time_rdm_band=temp0;
end    
